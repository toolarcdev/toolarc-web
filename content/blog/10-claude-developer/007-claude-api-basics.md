...
> - APIキーの発行手順や環境変数への安全な置き方は、続編の「[Claude APIキーの発行と環境変数への設定手順](/blog/claude-api-key-env-setup)」で詳しく扱います。料金やレート制限の詳細は「[Claude APIの料金とレート制限](/blog/claude-api-pricing-limits)」を参照してください
...
**Q1. Claude CodeとAPIは何が違いますか？**

Codeは対話型の実装支援、APIは自分のコードから呼び出す経路です。両方必要とは限りません。

**Q2. 401エラーが出ます。**

APIキーの誤り・失効が多いです。キー管理は[環境変数への設定](/blog/claude-api-key-env-setup)へ。

**Q3. 料金の目安はどこで見ますか？**

[Claude APIの料金とレート制限](/blog/claude-api-pricing-limits)へ進んでください。

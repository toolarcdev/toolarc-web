...
**粒度が違うので、置き換えではありません**。MCP対応の接続先は内部でAPIを呼び出していることも多く、MCPはAPIと競い合う規格ではなく、その上に載る取り決めと考えると分かりやすいところです。社内APIや外部の公開APIを、自分のコードからRESTで直接呼ぶか、MCPでつなぐかの判断は、[MCPとREST API連携の違い](/blog/mcp-vs-rest-api-comparison)で扱っています。

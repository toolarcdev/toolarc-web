...
> - ローカル起動は`command`・`args`・必要なら`env`を接続先の案内に合わせて書き、認証情報はプレースホルダにします。
...
認証情報はプレースホルダのまま置き、実際のトークンは書き込まないでください。Extensions側で入力した機密項目（sensitive設定）は、macOSのキーチェーンやWindowsの資格情報マネージャーなど、OSの安全な保存領域に暗号化して保存される案内になっています。設定ファイルでは同じ暗号化は働きません。

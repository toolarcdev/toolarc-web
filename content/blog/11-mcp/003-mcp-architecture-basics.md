...
仕組みが分かっても、外部ツールをつなぐ用途がいまなければ、設定画面を開く必要はありません。この記事で扱うのは仕組みまでで、実際の接続手順や設定ファイルの書き方は範囲外です。次に自分用の最小Serverを書いてToolを1つ公開したい場合は、[MCP Serverの作り方入門](/blog/mcp-server-build-basics)を確認してください。

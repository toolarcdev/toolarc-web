...
# 10. Series 入口 概念図（chatgpt-account-migration-series）

## ファイル

- `series-entrypoint-diagram.png`（4:3・軽量化済・約132KB）

## 用途

- `/series/chatgpt-account-migration-series` ヘッダ直下の概念図
- 「同じアカウントの機種変更」と「別アカウントへの移行」の分岐を抽象アイコンで示す（文字なし）
- 実UIの代替ではない

## 生成メモ

- 2026-07-21 generate-blog-image（白背景・`#60a5fa`・文字なし）
- 構図: 上部ハブ → 左＝スマホ＋同一クラウド、右＝分岐する2クラウド

## alt text

```md
同じアカウントでの機種変更と、別アカウントへの移行が分かれることを示す概念図
```

## 配置

- `lib/series/series.ts` → `chatgpt-account-migration-series` の `heroImage`
- 表示: `app/series/[slug]/page.tsx` ヘッダ直下
- **2026-07-21 差し替え**: 低品質 `series-entrypoint-diagram.png` をやめ、`pattern-comparison-both.png` を hero に流用（再生成せず）

---

# 11. チェックリスト記事 分岐概念図（chatgpt-account-migration-checklist）

## ファイル

- `checklist-same-vs-other-account-diagram.png`（4:3・軽量化済・約14KB）

## 用途

- `/blog/chatgpt-account-migration-checklist` 本文「まず同じアカウントか別アカウントか」H2直後
- 同じアカウント継続 vs 別アカウント分岐の抽象図（文字なし）
- OG 暫定としても同ファイルを参照（理想は16:9専用OGだが、default欠落より優先）

## 生成メモ

- 2026-07-21 generate-blog-image（白背景・`#60a5fa`・フラット・文字なし）
- 構図: 上部ノード → 左＝スマホ＋同一クラウド、右＝2クラウド分岐

## alt text

```md
同じアカウントでの移行と、別アカウントへの移行が分かれることを示す概念図
```

## 設定箇所

- `lib/blog/posts.ts` → `imageBasePath` を `040-chatgpt_account_migration` へ変更
- `ogImage` / 本文とも **`pattern-comparison-both.png` 流用**（2026-07-21）

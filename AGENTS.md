...
- **Phase2-0**: 売上より ASPクリック信号。卒業＝ASPクリック累計≥10 → カレンダー Phase2
- **方針B**: 量産維持、シリーズ負債は水曜原則2単位（公開は止めない）
- 出力評価の優先順:
  1. 検索意図に一致している
  2. 初心者が再現できる
...
0. 現行フェーズを確認する（`docs/plan/phase-now.md`）。2-0中は刈り取り・導線計測・負債並行を量産と両立する
1. 読者の悩みを1文で定義する
...
- 基本文体は「です・ます調」（敬体ベース＋文末多様化。詳細は `writing-rules.md`）、一人称は「筆者」

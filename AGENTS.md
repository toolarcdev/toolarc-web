...
| source.md 作成 | `.cursor/skills/create-source-md/SKILL.md` | 構成・新規公開の例外ゲート・Claude制約。本文初稿はしない |
...
2. **source.md 作成** → Skill `create-source-md`（Vault 配置・ペルソナ・新規公開の例外ゲート・Claude制約。本文初稿・公開はしない）
...
依頼を **データ / 表示(UI) / 計測** に分解し、依頼に書かれた層だけ触る。

実装前に次を確認する（矛盾・曖昧・理由付けのすり替えを防ぐ）:

1. **既存の不変条件と矛盾しないか** — 関連する正本（例: `affiliate-registry.md`、対象コンポーネントのコメント）を読み、依頼と矛盾する変更をしない
2. **解釈が2通り以上あるか** — ある場合は **1行で解釈を提示し、確認が取れるまで実装しない**（Agent モードでも同様）
3. **計測の話が UI 変更の理由になっていないか** — 既存の計測・ローテで足りるなら UI は触らない

依頼にないファイル（例: プール変更だけなのに `*AffiliateSlot.tsx`）は、**明示指示がない限り変更しない**。

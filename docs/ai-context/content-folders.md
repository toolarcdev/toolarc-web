...
`21-` 以降は未使用の整数を採番する（次の新束は `23-…`）。1本だけの束でもよい（後から同フォルダへ追記）。
...
```
公開後（⑥）
  → 日次: 候補マスターで content_folder を series:* / topic:* / standalone 等へ（新規 B/21/22 は topic:* または standalone）
  → 水曜 #5: シリーズ化スキャン（B/21/22 とレガシー20。GSC・相互リンクの塊 → series:candidate）
  → 水曜 #6: 重負債（既存 Hub stale / 20→A|B 昇格 / 統合・301）→ 必要なら①
  → series 化①: Hub 新規 + series.ts + 関連 MD を新 A（または既存 A）へ移動 + Hub に全スポーク反映（同一 PR）
  → 単発のまま: B/21/22 に残し standalone（昇格キューに入れない）
```
...
| 層 | プレフィックス | 例 |
|----|----------------|-----|
| **A/B**（`20` 以外。`21-`／`22-` 含む） | `{3桁連番}-`（フォルダ内、Hub = `001-`） | `08-new-domain-seo` → `001-`〜 / `21-foo` → `001-`〜 |
...
- B／`21-`／`22-` はフォルダ内公開順またはテーマ内の読む順で連番
...
| `standalone` | B／`21-`／`22-` 等に公開・series 化予定なし（または勝ち待ち）。昇格キュー外 |
...
5. ⑥ 水曜 #5: **シリーズ化スキャン**（B/21/22・レガシー20。候補付与）

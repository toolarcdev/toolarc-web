...
【導線リライトバッチ】
対象 slug（既存のみ）:
- <slug1>（追加: 1行〜短段落。④差分文案を添付）
- <slug2>
- ...

リンク先（確定 slug を明記）:
- 周辺収益記事: /blog/<peripheral-slug>
- Rakurin 収益記事: /blog/<rakurin-slug>（公開済みのみ）

【ルール】
- 比較Series4本: 直アフィ禁止。内部リンク（弱）のみ
- ④の差分文案どおり最小変更。全文書き換えしない
- アフィリエイトURLは④文案に従う（ASP管理画面条件の記事転載禁止）
...
- 判定基準の正本は Vault 改善施策ノート §4.3.1（**必須合否**＋**任意観察**）。**合否の最優先は読者最優先**（ペルソナ充足・「今知りたいか／今クリックしたいか」・着地完結・リンク動機・用語公開面・出口の場所）。サブ情報 +α は任意。冒頭リンクは一律禁止しない。**ペルソナ2〜3は source.md 作成時から必須**（Output Contract 項目1）。**です／ます3連続は必須修正にしない**（任意。ブロックまたぎはさらに緩く）
...
- Phase1〜Phase4 および Phase2-0〜2-5 の目標、撤退ライン、判断基準（正本: docs/plan/phase-now.md）
...
- phase-now.md / seo-goals.md との差分があれば改訂案を出す

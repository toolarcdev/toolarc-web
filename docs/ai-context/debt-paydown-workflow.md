...
最終更新: 2026-07-08 18:42（方針B: 量産維持・水曜重負債を原則2単位・赤域でも公開停止しない）  
用途: `20-investigate-something` 先公開フローで溜まる「後払い負債」の払い方。①（公開）・⑥（日次/週次）が参照。

関連: [`content-folders.md`](content-folders.md)、[`chat-operations.md`](chat-operations.md)、[`lib/series/series.ts`](../../lib/series/series.ts)、[`docs/plan/phase-now.md`](../plan/phase-now.md)
...
**方針B（現行・`phase-now.md`）**: 新規公開ペースは維持する。追いつかない分は水曜単位数と赤域時の整理①追加枠で払う（公開スキップはしない）。
...
毎週水曜の週次メンテで **原則2単位** 実施（方針B）。同一シリーズの同時大量昇格はしない。単位が足りない週は最低1単位、余力があれば3単位まで可（通常は2）。
...
```text
【負債払い（方針B・原則2単位）】
単位1: 種別 Hub更新|昇格PR|逆リンク / シリーズ / slug一覧
単位2: 種別 Hub更新|昇格PR|逆リンク / シリーズ / slug一覧
...
| `20` 総数 **≥50本** | 黄 | 48時間以内に①を整理 PR（P0/P1）へ1回差し替え。**公開は継続** |
| `20` 総数 **≥70本** | 赤 | 赤指標が2つ以上なら **公開は継続**し、翌営業日に整理①を **追加1枠**（方針B） |
| 週次純増 **≥+15本** | 黄 | 次回水曜 #6 を2単位厳守（余力あれば3） |
| 週次純増 **≥+25本** | 赤 | 赤指標が2つ以上なら水曜待たず整理①を追加1枠（公開は止めない） |
| 単発確定（シリーズ化予定なし） | — | 公開時に `standalone`。昇格キューに入れない |

**運用注釈（方針B）**

- 現ペース（日次 +3本 × 週7日 ≒ **+21本/週**）では、週次純増 +10〜+21 は通常範囲
- `20` 総数は公開速度の結果も拾うため、**単独では新規公開停止の根拠にしない**
- **公開スキップはしない**。赤域の対処は「整理①の追加枠」と「水曜2単位」で払う
- 黄/赤は「正常/異常」ではなく **ベースライン超過の段階的通知**
- フェーズ文脈: [`docs/plan/phase-now.md`](../plan/phase-now.md)

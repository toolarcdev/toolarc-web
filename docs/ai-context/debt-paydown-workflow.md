...
最終更新: 2026-08-04 19:33（評価フェーズ: 後付け series・B/21/22 直置き・`20` 新規禁止）  
...
- 勝ちが見えるまで Hub／`series.ts` は作らない。新規は **既存 B**（`21-cursor-models` 含む）または **新 `23-…`** に直置きし、同ジャンルは相互リンク（`22-game-dev-js` は `12-game-dev-js` へ昇格済）
...
- B／`21-`／`22-` クラスタの series 化タイミング（データ待ち。負債というより判断待ち）
...
| # | 作業 | 条件 |
|---|------|------|
| 1 | **置き場を確定** | [`content-folders.md`](content-folders.md): 既存 A / 既存 B / 新 `23-…`。**`20` へ置かない** |

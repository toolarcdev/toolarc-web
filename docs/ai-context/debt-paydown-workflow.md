...
- 勝ちが見えるまで Hub／`series.ts` は作らない。新規は **既存 B**（`21-cursor-models` 含む）または **新 `22-…`** に直置きし、同ジャンルは相互リンク
...
| # | 作業 | 条件 |
|---|------|------|
| 1 | **置き場を確定** | [`content-folders.md`](content-folders.md): 既存 A / 既存 B / 新 `22-…`。**`20` へ置かない** |

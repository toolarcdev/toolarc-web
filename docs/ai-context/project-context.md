...
- **シリーズ構造**: Hub（入口）＋ Spoke を基本形にする。新規は既存 B または新 `22-…` に直置きし、勝ちが見えてから後付けで series／Hub 化する（`21-cursor-models` は使用済み。`20` は量産負債のみ。正本: [`content-folders.md`](content-folders.md)）

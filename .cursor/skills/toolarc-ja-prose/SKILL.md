...
1. 敬体ベース＋文末多様化（**段落末は敬体**／体言止めは段落途中の山場／ゼロ回避。です／ます3連続の切断は**任意**で必須ではない・**免責は対象外**。リズム切断だけの短文メタを増やさない。詳細は writing-rules）
...
8. **抽象語の役割別ばらし** — 「経路」「入り口」など、意味は通るが記事内で同じ抽象語が役割を変えずに続くときは、**全部を同一置換語に揃えない**。総称・対比ラベル・「どちらで」など**文中の役割ごとに**日常語へばらす（例: 追加方法の選択／方法／足し方／Extensionsから足す場合／設定ファイルを書く場合／どちらで／Extensions側／設定ファイルでは）。カタカナ英語（ルート等）は、隣記事の専門語と混ざるなら避ける。**使用回数の閾値・上限は定めない**（人間の読み味判断。機械カウントで不合格にしない）

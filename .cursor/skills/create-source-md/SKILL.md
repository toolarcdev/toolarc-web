...
インプット作成時、**任意でペルソナを設定してよい**。インプットのペルソナ・記事の仕事は**一つの案**であり確定ではない。**確定は source.md 側の Agent** が行う（案の丸写し必須ではない。2〜3に整える）。ペルソナ案は経験・迷い・検索着地の状況で書く。**他記事／Hub既読を軸にしない**（正本: `references/article-input.md`。例: 「前の手順記事を読んだ人」「Hubの地図だけ見た人」は不採用）。インプットに既読軸があれば Agent は立て直す。

### 人と Agent の分担

| 担当 | 内容 |
|------|------|
| **人（インプット）** | 必須6＋任意。任意でペルソナ・記事の仕事の**案**を書いてよい（既読軸は不可。`article-input.md`）。根拠・境界・導線・戦略制約は推測禁止 |
| **Agent（本 Skill）** | ペルソナ2〜3・記事の仕事を**確定**（インプットに案があれば材料。既読軸なら立て直す。無ければ立てる）。Output Contract、H2（箇条）、ゲート文案（通過済は短文）、**Claude は記事差分のみ** |
...
| 6 | ペルソナ 2〜3 | 各: **呼び名／知りたいメイン／着地の求め／この記事で充足する不足**。公開本文へ転載しない。**source.md 側の Agent が確定**。呼び名が他記事／Hub既読を軸にしていない（インプット案が既読軸なら立て直す） |
| 7 | Output Contract 8項目 | writing-rules どおり。想定読者1行＝ペルソナ要約。H2 最大5前後。収益導線はインプット方針と一致 |
| 8 | 構成骨格の詳細 | H2ごとに「書くこと／書かないこと」を**箇条**（各おおむね3〜7項目）。初稿級の散文に膨らませない。抽象ラベル単独見出し禁止。**公開面文案（description／導入／今日の結論／本論の約束）は他記事既読を前提にしない**（writing-rules。④依頼メモにも1行） |
...
- **正本は source.md のペルソナ2〜3**（上表。インプット案は材料。既読軸の案は不採用）。知りたいメインの充足判定もここ

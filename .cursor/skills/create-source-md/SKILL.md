...
description: >-
  ...
  folder: personas, Output Contract, §5.2 gate, cannibal boundaries, evidence
  bar, Claude.ai constraints, and handoff notes for ④/①. Use when the user asks
  for source.md作成, 構成正本, 記事のsource, slot ⑤ source, or §5.2ゲート用source.
  Not for writing article body drafts, posts.ts, publish, L1 review, or PR.
---
...
| 評価ゲート | Vault `toolarc.jp_評価フェーズ移行_現状と方針検討_2026-07-31` §5.2・§5.3 | 新規例外ゲート4点・所属 Hub/SubHub |
...
5. **完了報告**: ファイルパス／slug／§5.2 4点の可否／④へ渡す1行／①へ渡す1行
...
| 3 | タイトル＋description案 | 公開時 title 案。description **120〜160字を狙う案**（①で再実測）。`――` 禁止・区切りは `｜` または `：` |
| 4 | 記事の仕事／やらないこと | 1段落の仕事＋本文に入れない箇条 |
| 5 | 悩み／問題／原因／対策／伝えたいこと | 5項目すべて埋める |
| 6 | ペルソナ 2〜3 | 各: **呼び名／知りたいメイン／着地の求め／この記事で充足する不足**。想定した全員のメインが構成・結論で賄えること。公開本文へ転載しない旨を書く |
| 7 | Output Contract 8項目 | writing-rules どおり（想定読者1行はペルソナと整合。構成案は H2 最大5前後） |
| 8 | 構成骨格の詳細 | H2ごとに「書くこと／書かないこと」。抽象ラベル単独見出し禁止 |
| 9 | §5.2-1 | 既存URLでは意図をカバーできない理由（統合・リライト・SubHub で不可） |
| 10 | §5.2-2 根拠バー | 実測／一次ログ **または** 調査・公式Docs・二次分析・SERP/GSC を本文で辿れる形。薄味枠埋め禁止 |
| 11 | §5.2-3 カニバル | 1段落＋**隣記事の役割境界・転記禁止表** |
| 12 | §5.2-4 キュー | week-queue の Q・target・status を明記（未掲載ならユーザー確認） |
...
- 初稿用 YAML Frontmatter 雛形（title / description / date プレースホルダ / tags / site / target）
...
- path: <Vault path>
- slug: <slug>
- §5.2: 1既存URL / 2根拠 / 3カニバル / 4キュー = 通過 or 要確認

...
1. **L1レビュー（項目別合否）**: 次を ✅／要修正 で判定する（正本は改善施策ノート §4.3.1）
   ...
   - 文体: 敬体ベース＋文末多様化（同一文末です／ますの**3連続禁止**）。一人称は通常省略し、「筆者」は実測・失敗談・確認範囲・未検証の境界を示す必要がある文だけに限定。山場の体言止め・短い常体は `writing-rules.md` に従い許容
   ...
   - 内部判定用の一次情報マーカーが最低1つあるか（実測数値＋取得日／環境／失敗談）。無い場合は確認日・参照元・未検証範囲が具体的か。公開見出しに「一次情報」「筆者の確認範囲」をそのまま出していないか
   - 見出しスキャニングテスト（見出しだけで「何が分かるか／何をするか」を推測できるか。抽象ラベル単独を避ける）
   - 免責 ≠ 見出し: `## 免責` / `### 免責` / 見出し文言に「免責」を含むものがないこと。あれば見出しを外し `---`＋段落（本命）または `> **免責**` 引用に直す（StickyTOC ノイズ防止）
   - `――` / 同型 `——` がないこと（文中の間、および見出し・`title`）。文中は句点分割・読者への問い等へ。見出しは `｜`（または `：`）へ。本文箇条の単独 `—` 区切りは必須対象外
   - 文末の丸括弧が `（…）。` になっていないか（括弧で文を終える場合は `（…）`）
   - 引用の直後に句点を重ねていないか（`「…」。` → `「…」`。括弧と同じ）
   - 括弧の入れ子を基本的に避けているか。`（「…」）` / `「(…)`」 → 括弧内の強調は `"…"` / `` `…` ``。未公開は `「準備中」`

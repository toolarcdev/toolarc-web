...
Skill = 手順。判定基準の正本は改善施策ノート §4.3（Output Contract 8項目＋検索意図一致＋誇大表現）。
矛盾したら **Vault 正本（改善施策ノート §4.3・§8）を優先**し、本 Skill を追従更新する。

## 正本と同期

| 層 | パス | 役割 |
|----|------|------|
| 判定基準の正本 | Vault `06_toolarc-business/202607/ToolArc生産性向上_改善施策と活用機能（2026-07）.md` §4.3 | L1チェックリスト・レビュー→反映テンプレ |
...
**やらない**: `posts.ts` / `series.ts` 登録、`npm run build`、公開日確定（すべて①＝`publish-article`）、全文初稿の作成（④のProduceは別）、GSC（②）、PoE2/api

## 手順

開始時: `Get-Date -Format "yyyy-MM-dd HH:mm"` → 編集した Vault ファイルの `Last Updated`／記録の実施日時に使う（手入力・推測禁止）

1. **L1レビュー（項目別合否）**: 次を ✅／要修正 で判定する
   - Output Contract 8項目: 想定読者／検索意図／主・副KW／結論3〜5点／構成（H2最大5前後）／収益導線／内部リンク／免責
   - 追加観点: 文体（です・ます／一人称「筆者」）／誇大表現・未確認仕様の断定なし／slug 3点一致（ファイル名・frontmatter・依頼指定）／内部リンクは `/blog/slug` 相対形式／rich-toc なら `<!-- embed:* -->` 維持
...
- 判定基準の正本: Vault 改善施策ノート §4.3（迷ったときだけ該当節を読む。全文は読まない）
...
- token節約: 対象記事・当日AI-log・§4.3以外は読まない。広範囲探索は実行前に確認

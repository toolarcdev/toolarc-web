...
Skill = 手順。判定基準の正本は改善施策ノート **§4.3.1 L1判定基準**（Output Contract 8項目＋リズム／空句／一次情報＋誇大表現＋読者最優先＋任意観察）。
...
1. **L1レビュー（項目別合否）**: 次を ✅／要修正 で判定する（正本は改善施策ノート §4.3.1 **必須**）。詳細基準は `writing-rules.md` / `llm-forbidden-phrases.md` を開き、本Skillへコピーしない。
   ...
   - 読者最優先（§4.3.1 項目13／writing-rules「読者最優先」）
     - **ペルソナ充足（必須）**: source.md にペルソナ2〜3があればそれを正とする。無ければ想定ペルソナ2〜3を立てる。各々の知りたいメイン・着地の求め・不足の充足を見る。全ペルソナのメインが揃っていなければ要修正。境界外は充足義務外。AI-logの項目別合否にペルソナ名と過不足を短く残す（source由来かL1補完かも1語）
     - **内部リンク**: 着地意図の完結／リンクの動機（「このリンクを今クリックしたいか」）／用語の公開面／出口の場所。**冒頭リンクの一律禁止はしない**。Hub／シリーズ入口は地図としての誘導を不合格にしない

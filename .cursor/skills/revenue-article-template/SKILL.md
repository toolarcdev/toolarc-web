...
4. 内部判定用の一次情報（実測値＋取得日／使用環境／失敗談）を確認する。無ければ捏造せず、確認日・参照元・未検証範囲を具体的に書く。公開見出しに「一次情報」「筆者の確認範囲」をそのまま出さない。
...
6. **根拠・体験** — 事実は該当する本文へ自然に埋める。環境・確認日・参照元・未検証範囲をまとめる必要がある場合だけ、免責直前に読者向けの見出しで置く
...
- `writing-rules.md` の文末多様化・空句・一次情報要件を満たし、同一文末（です／ます）が3連続していない
- 「筆者」は実測・失敗談・確認範囲・未検証の境界を示す文だけに限定する
- 見出しだけで「何が分かるか／何をするか」が推測でき、抽象ラベル単独になっていない
- 文末が丸括弧なら `（…）` で閉じ、`（…）。` にしない
